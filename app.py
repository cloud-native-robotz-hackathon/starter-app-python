...
def run():
    ...
    # response = requests.get('http://' + application.config['ROBOT_API'] + ':5000/distance' + '?user_key=' + application.config['API_TOKEN'], verify=False)  
    # response = requests.get(application.config['URI'] + '/power' + '?user_key=' + application.config['API_TOKEN'],verify=False)
...
def status():
    response = requests.get(application.config['URI'] + '/remote_status?user_key=' + application.config['API_TOKEN'], verify=False)
